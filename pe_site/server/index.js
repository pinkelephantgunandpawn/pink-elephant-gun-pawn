...
const SHIPPING_DEFAULTS={
  small:{weight:1.5,length:10,width:8,height:4},
  medium:{weight:5,length:16,width:12,height:8},
  large:{weight:12,length:22,width:18,height:14},
  oversize:{weight:30,length:30,width:24,height:18},
  guitar:{weight:20,length:48,width:20,height:8},
  console:{weight:12,length:18,width:14,height:8}
};
const SHIPPING_PROFILES=new Set(['auto','small','medium','large','oversize','guitar','console']);
function inferShippingProfile(inv){
  const requested=String(inv.shipping_profile||'auto').trim().toLowerCase();
  if(requested&&requested!=='auto'&&SHIPPING_PROFILES.has(requested))return requested;

  const title=String(inv.title||'').toLowerCase();
  const cat=String(inv.category||'').toLowerCase();
  const text=`${title} ${cat}`;

  // Long instruments get their own dimensional profile.
  if(/\b(guitar|bass guitar|electric bass|acoustic|les paul|strat(?:ocaster)?|tele(?:caster)?|banjo|mandolin)\b/.test(text))return 'guitar';

  // Bulky music gear and other clearly large items.
  if(/\b(amplifier|amp\b|combo amp|speaker cabinet|cabinet|studio pro|bandit|keyboard|digital piano|drum kit|floor tom|subwoofer|receiver)\b/.test(text))return 'oversize';

  // Small music/electronic accessories should never inherit "guitar" just because the category is Music.
  if(/\b(pedal|stompbox|chorus|delay|distortion|overdrive|fuzz|tuner|footswitch|pickup|strings|cable|adapter|earbud|earbuds|headphone|headphones|mouse|charger|remote)\b/.test(text))return 'small';

  if(/\b(console|playstation|xbox|nintendo|switch|ps[345]\b)\b/.test(text))return 'console';
  if(/\b(laptop|tablet|camera|router|modem|controller|microphone|mic\b)\b/.test(text))return 'medium';
  if(/\b(tv|television|monitor|vacuum|pressure washer|miter saw|table saw|compressor|generator|tool chest|large speaker)\b/.test(text))return 'oversize';
  if(/\b(drill|impact|sawzall|circular saw|sander|grinder|power tool)\b/.test(text))return 'medium';

  if(/jewel|collect|accessor|coin|card/.test(cat))return 'small';
  if(/gaming/.test(cat))return 'console';
  if(/electronic/.test(cat))return 'medium';
  if(/music|instrument/.test(cat))return 'medium';
  if(/tool|sport/.test(cat))return 'large';
  return 'medium';
}
function estimatedParcelForInventory(inv){
  ...
  if(saved.every(v=>Number.isFinite(v)&&v>0))return {weight:saved[0],length:saved[1],width:saved[2],height:saved[3],estimated:false,profile:'exact'};
  const key=inferShippingProfile(inv);
  const p={...SHIPPING_DEFAULTS[key]};
  // If staff knows only the weight, preserve it while still estimating the box size.
...
app.post('/api/public/shipping-rates',checkoutLimit,async(req,res)=>{
  ...
  try{
    ...
    res.json({rates,test_mode:rates.every(x=>x.test),subtotal_cents:subtotalCents,parcel_note:`Estimated shipping uses exact saved package data when available; otherwise it uses the item's Shipping Size or an automatic title/category size guess. A ${'$'}${(SHIPPING_ESTIMATE_BUFFER_CENTS/100).toFixed(2)} packing/estimate buffer is included.`});
...
app.post('/api/inventory/:id/duplicate',auth,requireRole('manager'),async(req,res)=>{
  try{
    const {rows}=await pool.query(`INSERT INTO inventory(title,category,quantity,cost_cents,price_cents,price_label,sku,item_type,low_stock,description,image_url,image_urls,condition,sale_price_cents,featured,regulated,public_visible,shipping_profile,shipping_weight_lb,shipping_length_in,shipping_width_in,shipping_height_in)
      SELECT title || ' COPY',category,quantity,cost_cents,price_cents,price_label,NULL,item_type,low_stock,description,image_url,image_urls,condition,sale_price_cents,false,regulated,false,shipping_profile,shipping_weight_lb,shipping_length_in,shipping_width_in,shipping_height_in
